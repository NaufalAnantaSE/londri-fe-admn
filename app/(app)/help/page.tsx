...
function FaqItem({ faq, highlight }: { faq: Faq; highlight: string }) {
  ...
  return (
    <div className="border-t border-slate-100 dark:border-slate-800 first:border-t-0">
      <button onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-3 py-3 text-left">
        <span className="flex-1 text-sm font-medium">{mark(faq.q, highlight)}</span>
        <CaretDown size={16} className={`shrink-0 text-slate-400 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>
      <div className={`grid transition-[grid-template-rows] duration-200 ease-out ${open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
        <div className="overflow-hidden">
          <p className="pb-3 text-sm leading-relaxed text-slate-500 dark:text-slate-400">{faq.a}</p>
        </div>
      </div>
    </div>
  );
...
function mark(text: string, q: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="rounded bg-sky-100 px-0.5 text-sky-800 dark:bg-sky-900 dark:text-sky-200">{text.slice(idx, idx + q.length)}</mark>
      ...
      <div className="space-y-4 p-4">
        <div>
          <div className="mb-1 flex items-center gap-2">
            <Question size={20} weight="duotone" className="text-sky-500" />
        ...
        {filtered.map((s) => (
          <section key={s.id} className="glass rounded-2xl border border-slate-200/60 dark:border-slate-800 p-4">
